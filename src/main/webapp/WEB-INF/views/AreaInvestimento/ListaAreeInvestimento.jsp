...
<meta charset="ISO-8859-1">
	<title>Lista Aree Investimento</title>
	...
	<script type = "text/javascript">
		function deleteConfirm() {
			...
			if (confirm("Eliminare l'area investimento selezionata?")) {
				return true;
  			} else {
   				return false;
  			}
...
<body>
	<div class="container">
	  	...
	  	<table class="table table-striped">
		   	...
		   	<tbody>
		    	<c:forEach items="${elencoAreeInv}" var="elenco">
		     	<tr>
		      		<td>${elenco.codice}</td>
		      		<td>${elenco.areaInvestimento}</td>
		      		<td>${elenco.oAnnoContabile.descrizione}</td>
		      		<td>
		      			<spring:url value="/AreaInvestimento/EditAreaInvestimento/${elenco.idAreaInvestimento}" var="editURL" />
		      			<a href="${editURL}" role="button" class="btn btn-primary">Modifica</a>
		      		</td>
		      		<td>
		      			<spring:url value="/AreaInvestimento/DeleteAreaInvestimento/${elenco.idAreaInvestimento}" var="deleteURL" />
